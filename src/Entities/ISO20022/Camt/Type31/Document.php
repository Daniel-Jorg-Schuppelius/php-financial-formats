...
namespace CommonToolkit\FinancialFormats\Entities\ISO20022\Camt\Type31;
...
class Document implements CamtDocumentInterface {
    ...
    protected ?string $rejectionReasonCode = null;
    protected ?string $rejectionReasonProprietary = null;
    ...
    public function __construct(
        ...
        ?string $rejectionReasonProprietary = null,
        ...
        $this->rejectionReasonProprietary = $rejectionReasonProprietary;
    ...
    public function getRejectionReasonProprietary(): ?string {
        return $this->rejectionReasonProprietary;
    }

    public function getRejectionReason(): ?string {
        return $this->rejectionReasonCode ?? $this->rejectionReasonProprietary;
    }
    ...
    public function toXml(CamtVersion $version = CamtVersion::V07): string {
        ...
        $rjctInvstgtn = $dom->createElement('RjctInvstgtn');
        $root->appendChild($rjctInvstgtn);

        // Assgnmt (Assignment)
        $assgnmt = $dom->createElement('Assgnmt');
        $rjctInvstgtn->appendChild($assgnmt);
        ...
        // Justification
        if ($this->rejectionReasonCode !== null || $this->rejectionReasonProprietary !== null) {
            $justfn = $dom->createElement('Justfn');
            $rjctInvstgtn->appendChild($justfn);

            $rjctnRsn = $dom->createElement('RjctnRsn');
            $justfn->appendChild($rjctnRsn);

            if ($this->rejectionReasonCode !== null) {
                $rjctnRsn->appendChild($dom->createElement('Cd', htmlspecialchars($this->rejectionReasonCode)));
            } elseif ($this->rejectionReasonProprietary !== null) {
                $rjctnRsn->appendChild($dom->createElement('Prtry', htmlspecialchars($this->rejectionReasonProprietary)));
            }

            if ($this->additionalInformation !== null) {
                $justfn->appendChild($dom->createElement('AddtlInf', htmlspecialchars($this->additionalInformation)));
            }
        }
