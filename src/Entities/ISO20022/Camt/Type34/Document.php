...
namespace CommonToolkit\FinancialFormats\Entities\ISO20022\Camt\Type34;
...
class Document implements CamtDocumentInterface {
    ...
    // Duplicate content
    protected ?string $duplicateContent = null;
    protected ?string $duplicateContentType = null;
    ...
    public function __construct(
        ...
        ?string $duplicateContent = null,
        ?string $duplicateContentType = null
    ) {
        ...
        $this->duplicateContentType = $duplicateContentType;
    }
    ...
    public function getDuplicateContent(): ?string {
        return $this->duplicateContent;
    }

    public function getDuplicateContentType(): ?string {
        return $this->duplicateContentType;
    }
    ...
    public function toXml(CamtVersion $version = CamtVersion::V07): string {
        ...
        $dplct = $dom->createElement('Dplct');
        $root->appendChild($dplct);

        // Assgnmt (Assignment)
        $assgnmt = $dom->createElement('Assgnmt');
        $dplct->appendChild($assgnmt);
        ...
        if ($this->duplicateContent !== null) {
            $dplctData = $dom->createElement('Dplct');
            $dplct->appendChild($dplctData);

            $prtryData = $dom->createElement('PrtryData');
            $dplctData->appendChild($prtryData);

            if ($this->duplicateContentType !== null) {
                $prtryData->appendChild($dom->createElement('Tp', htmlspecialchars($this->duplicateContentType)));
            }

            $data = $dom->createElement('Data');
            $prtryData->appendChild($data);
            $data->appendChild($dom->createCDATASection($this->duplicateContent));
        }
