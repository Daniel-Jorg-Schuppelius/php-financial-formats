...
class CamtParser extends Iso20022ParserAbstract {
    ...
    private static function parseTransaction053WithPrefix(DOMXPath $xpath, DOMNode $entry, string $defaultCurrency, string $p): ?Camt053Transaction {
        ...
        if ($txDtls) {
            ...
            $debtor = self::parsePartyIdentification($xpath, $txDtls, "RltdPties/{$p}Dbtr", $p);
            $creditor = self::parsePartyIdentification($xpath, $txDtls, "RltdPties/{$p}Cdtr", $p);
     ...
     */
    private static function parsePartyIdentification(DOMXPath $xpath, DOMNode $context, string $partyPath, string $p): ?PartyIdentification {
