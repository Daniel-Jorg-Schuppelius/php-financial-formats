...
class Camt035Generator extends CamtGeneratorAbstract {
    ...
    private function addProprietaryData(Document $document): void {
        if ($document->getProprietaryData() === null) {
            return;
        }

        $this->builder->addElement('PrtryData');
        if ($document->getProprietaryType() !== null) {
            $this->builder->addChild('Tp', $this->escape($document->getProprietaryType()));
        }
        $this->builder->addChild('Data', $this->escape($document->getProprietaryData()));
        $this->builder->end();
    }
}
