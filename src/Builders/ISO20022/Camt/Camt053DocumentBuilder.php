 ...
 */
final class Camt053DocumentBuilder {
    private string $id;
    ...
    public function build(): Document {
        ...
        if ($opening && !$closing) {
            ...
            if (
                abs($expected->getAmount() - $closing->getAmount()) > 0.01 ||
                $expected->getCreditDebit() !== $closing->getCreditDebit()
            ) {
                throw new RuntimeException(
                    "Opening- und Closing-Salden stimmen nicht überein. Erwartet: " .
                        ($expected->isDebit() ? '-' : '') . $expected->getAmount()
                );
            }
        }
     ...
     * @return Document Das konvertierte CAMT.053 Dokument
     */
    public static function fromMt940(Mt940Document $mt940, ?string $messageId = null): Document {
        return Mt940ToCamtConverter::toCamt053($mt940, $messageId);
    }
    ...
    public static function fromMt941(Mt941Document $mt941, ?string $messageId = null): Document {
        return Mt940ToCamtConverter::toCamt053(MtInterConverter::mt941ToMt940($mt941), $messageId);
    }
    ...
    public static function fromMt942(Mt942Document $mt942, ?string $messageId = null): Document {
        return Mt940ToCamtConverter::toCamt053(MtInterConverter::mt942ToMt940($mt942), $messageId);
    }
     ...
     * @return Document Das konvertierte CAMT.053 Dokument
     */
    public static function fromBankTransaction(BankTransaction $document, ?float $openingBalanceAmount = null, ?CreditDebit $openingBalanceCreditDebit = null, ?string $accountOwner = null): Document {
        return BankTransactionToCamt053Converter::convert($document, $openingBalanceAmount, $openingBalanceCreditDebit, $accountOwner);
    }
}
